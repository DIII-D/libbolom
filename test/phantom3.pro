function phantom3_parms
parms = [[.1,.65,.5,-0.05,-0.05,0],$
        [.1,.65,.5,-0.05,-0.05,0],$
        [.1,.65,.5,-0.05,-0.05,0],$
        [.1,.65,.5,-0.05,-0.05,0],$
        [10,.07,.04,-0.6,0.70,60]]
for i=0,3 do begin
...
print,g.mw,g.mh
  ...
  bigimage = phantom(650,650,ellipses=phantom3_parms())
  testimage = congrid(bigimage,65,65)

efitplot,shot,stime,testimage,matrmin=84.0,matrmax=254.0,matzmin=-160.0,matzmax=160.0,range=range,tl='Phantom 3'
status = call_external(lib,'bolom_proj',shot,testimage,proj)

save,file='phantom3.dat'
