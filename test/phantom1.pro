function phantom1_parms
parms = [[1,.65,.5,-0.05,0.048,0],$
        [1,.65,.5,-0.05,0.048,0],$
        [1,.65,.5,-0.05,0.048,0],$
        [1,.65,.5,-0.05,0.048,0]]
for i=0,3 do begin
...
print,g.mw,g.mh
  ...
  bigimage = phantom(650,650,ellipses=phantom1_parms())
  testimage = congrid(bigimage,65,65)

efitplot,shot,stime,testimage,matrmin=84.0,matrmax=254.0,matzmin=-160.0,matzmax=160.0,range=range,tl='Phantom 1'
status = call_external(lib,'bolom_proj',shot,testimage,proj)

save,file='phantom1.dat'
