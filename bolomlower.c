...
bolomfit_lower(shot,chans,nchans,inproj,sigma,r,nr,z,nz,xtens,ytens,efit,rax,zax,rxpt1,zxpt1,rxpt2,zxpt2,fitproj,fitimage,ftol,fret,iter)
int shot;
...

{
    ...
    if((f = fopen("lowercoefs.dat","r")) != NULL){
        for(i = 1;i <= n; ++i){
            if(fscanf(f,"%g\n",&undump[i]) != 1)break;
        }
        fclose(f);
...
float x[];
{
    ...
    float proj[CHANS];
    float val;
    float fsanity_splower();
    float wall;
    char *tmp;

    ++funcevals;
		++(*pow_its);
    wall = fsanity_splower(x);
    if(debug)
        splowerproj(proj,x,((funcevals%debug) == 0));
    else
        splowerproj(proj,x,0);
    val = 0.0;
